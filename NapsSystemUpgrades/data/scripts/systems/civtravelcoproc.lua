...
function getCargoBonuses(seed, rarity, permanent)
    math.randomseed(seed)

    local perc = 20 -- base value, in percent
    -- add flat percentage based on rarity
    perc = perc + (rarity.value + 1) * 5 -- add 0% (worst rarity) to +30% (best rarity)
    perc = perc / 100
    if permanent then perc = perc * 1.5 end

    local flat = 0 -- base value
    -- add flat value based on rarity
	local cargoplus = {200, 500, 1500, 4000, 10000, 25000, 70000}
    flat = cargoplus[rarity.value + 2] -- add +200 (worst rarity) to +70000 (best rarity)
	
    -- add randomized value, span is based on rarity
    if permanent then flat = flat * 5 end
    flat = round(flat)
...
function getHyperBonuses(seed, rarity, permanent)
    math.randomseed(seed)
    ...
    rdReach = math.max(1, rarity.value + 1) * 2 + rarity.value

    if rarity.value >= RarityType.Rare then
        dsReach = math.max(1, rarity.value + 1) * 2 + rarity.value
    end

    hsReach = math.max(1, rarity.value + 1) * 2 + rarity.value

    return rdReach, dsReach, hsReach
end

function onInstalled(seed, rarity, permanent)
    local reach = getHyperBonuses(seed, rarity, permanent)

    -- activate trader system
    ...
    -- activate cargo boost
    local cargoPerc, cargoFlat = getCargoBonuses(seed, rarity, permanent)
    addBaseMultiplier(StatsBonuses.CargoHold, cargoPerc)
    addAbsoluteBias(StatsBonuses.CargoHold, cargoFlat)

    -- transporter range
    local tpRange = getTransporterBonuses(seed, rarity, permanent)
    addAbsoluteBias(StatsBonuses.TransporterRange, tpRange)

    local rdReach, dsReach, hsReach = getHyperBonuses(seed, rarity, permanent)
    if permanent then
        addAbsoluteBias(StatsBonuses.RadarReach, rdReach)
        addAbsoluteBias(StatsBonuses.HiddenSectorRadarReach, dsReach)
...
function getName(seed, rarity)
    return "AA-Tech Improved Travelling Salesman Coprocessor Upgrade MK ${mark}"%_t % {mark = toRomanLiterals(rarity.value + 2)}
end

function getBasicName()
    return "Travelling Salesman Coprocessor /* generic name for 'AA-Tech Improved Travelling Salesman Coprocessor Upgrade MK ${mark}' */"%_t
end

function getIcon(seed, rarity)
    return "data/textures/icons/cash_processor.png"
end

function getEnergy(seed, rarity, permanent)
    local energy = 0

    -- add trading system energy consumption
    local trHistNum = getHistorySize(seed, rarity)
    local trEcoRange = getEconomyRange(seed, rarity, permanent)
    local trenergy = ((rarity.value + 2) * 0.0005 * trEcoRange * 1000 * 1000 * 1000 * 10) + (trHistNum * 5 * 1000 * 1000 * 10)
	--printlog("civtravelcoproc.lua: trading system energy=" .. tostring(trenergy))

    -- add cargo boost energy consumption
    local cargoPerc, cargoFlat = getCargoBonuses(seed, rarity, permanent)
    local caenergy = cargoPerc * 1.5 * 1000 * 1000 + cargoFlat * 0.01 * 1000 * 1000 *10
	--("civtravelcoproc.lua: cargo system energy=" .. tostring(caenergy))

    -- transporter range energy consumption
    -- no energy change for transporter system

    local rdReach, dsReach, hsReach = getHyperBonuses(seed, rarity, permanent)
    local hyenergy = rdReach * 12.5 * 1000 * 1000 + dsReach * 7.5 * 1000 * 1000 * 10
	--printlog("civtravelcoproc.lua: hyperspace system energy=" .. tostring(hyenergy))

	energy = trenergy + caenergy + hyenergy
	--printlog("civtravelcoproc.lua: SUM energy=" .. tostring(energy))
...
function getPrice(seed, rarity)
    ...
    -- add trading system price
    local trHistNum = getHistorySize(seed, rarity)
    price = price + (rarity.value + 2) * 4000 + 5000 * trHistNum * 2.0 ^ rarity.value;

    -- add cargo boost price
    local cargoPerc, cargoFlat = getCargoBonuses(seed, rarity, permanent)
    price = price + (cargoPerc * 100 * 450 + cargoFlat * 75) * 2.5 ^ rarity.value

    -- transporter range price
    local tpRange = getTransporterBonuses(seed, rarity, true)
    price = price + tpRange * 250

    local rdReach, dsReach, hsReach = getHyperBonuses(seed, rarity, permanent)
    price = price + rdReach * 3000 + dsReach * 450 * 2.5 ^ (rarity.value + 1)
...
function getTooltipLines(seed, rarity, permanent)
    ...
    -- add trading system description
    ...
    -- add cargo boost description
    local cargoPerc, cargoFlat = getCargoBonuses(seed, rarity, permanent)
    local baseCargoPerc, baseCargoFlat = getCargoBonuses(seed, rarity, false)
    if cargoPerc ~= 0 then
        table.insert(lines, {ltext = "Cargo Hold (relative)"%_t, rtext = string.format("%+i%%", round(cargoPerc * 100)), icon = "data/textures/icons/crate.png", boosted = permanent})
        table.insert(bonuses, {ltext = "Cargo Hold (relative)"%_t, rtext = string.format("%+i%%", round(baseCargoPerc * 0.5 * 100)), icon = "data/textures/icons/crate.png", boosted = permanent})
    end
    if cargoFlat ~= 0 then
        table.insert(lines, {ltext = "Cargo Hold"%_t, rtext = string.format("%+i", round(cargoFlat)), icon = "data/textures/icons/crate.png", boosted = permanent})
        table.insert(bonuses, {ltext = "Cargo Hold"%_t, rtext = string.format("%+i", round(baseCargoFlat * 0.5)), icon = "data/textures/icons/crate.png", boosted = permanent})
    end

    -- transporter
    local tpRange = getTransporterBonuses(seed, rarity, permanent)
    if permanent then
        table.insert(lines, {ltext = "Docking Distance"%_t, rtext = "+${distance} km"%_t % {distance = tpRange / 100}, icon = "data/textures/icons/solar-system.png", boosted = permanent})
    else
    ...
    local rdReach, dsReach, hsReach = getHyperBonuses(seed, rarity, permanent)
    if permanent then
        table.insert(lines, {ltext = "Jump Range"%_t, rtext = string.format("%+i", hsReach), icon = "data/textures/icons/star-cycle.png", boosted = permanent})
        table.insert(lines, {ltext = "Deep Scan Range"%_t, rtext = string.format("%+i", dsReach), icon = "data/textures/icons/radar-sweep.png", boosted = permanent})
        table.insert(lines, {ltext = "Radar Range"%_t, rtext = string.format("%+i", rdReach), icon = "data/textures/icons/radar-sweep.png", boosted = permanent})
    else
        table.insert(bonuses, {ltext = "Jump Range"%_t, rtext = string.format("%+i", hsReach), icon = "data/textures/icons/star-cycle.png"})
        table.insert(bonuses, {ltext = "Deep Scan Range"%_t, rtext = string.format("%+i", dsReach), icon = "data/textures/icons/radar-sweep.png"})
        table.insert(bonuses, {ltext = "Radar Range"%_t, rtext = string.format("%+i", rdReach), icon = "data/textures/icons/radar-sweep.png"})
    end

    if permanent then
        table.insert(lines, {ltext = "Excess Processing Power"%_t, rtext = "+117.2k", icon = "data/textures/icons/star-cycle.png", boosted = permanent})
        table.insert(lines, {ltext = "Socket Equivalent"%_t, rtext = "+1", icon = "data/textures/icons/star-cycle.png", boosted = permanent})
    else
    ...
    table.insert(lines, {ltext = "Subspace Distortion Protection"%_t, rtext = string.format("%+i", subdisprot), icon = "data/textures/icons/subspace-distortion-protection.png", boosted = permanent})
...
function getDescriptionLines(seed, rarity, permanent)
    ...
    table.insert(lines, {ltext = "AA-Tech Advanced Travelling Salesman Coprocessor"%_t})
    table.insert(lines, {ltext = "Slightly improves sensor systems "%_t, rtext = "", icon = "data/textures/icons/nothing.png", fontType = FontType.Normal, lcolor = ColorRGB(0.7, 0.7, 0.7)})
	table.insert(lines, {ltext = "Adds storage space and compresses goods"%_t, rtext = "", icon = "data/textures/icons/nothing.png", fontType = FontType.Normal, lcolor = ColorRGB(0.7, 0.7, 0.7)})
    table.insert(lines, {ltext = "Adds transporter system with increased range"%_t, rtext = "", icon = "data/textures/icons/nothing.png", fontType = FontType.Normal, lcolor = ColorRGB(0.7, 0.7, 0.7)})
    table.insert(lines, {ltext = "Boosts effectiveness of your hyperspace engine"%_t, rtext = "", icon = "data/textures/icons/nothing.png", fontType = FontType.Normal, lcolor = ColorRGB(0.7, 0.7, 0.7)})
...
function getComparableValues(seed, rarity)
    ...
    local cargoPerc, cargoFlat = getCargoBonuses(seed, rarity, true)
    local baseCargoPerc, baseCargoFlat = getCargoBonuses(seed, rarity, false)
    if baseCargoPerc > 0 then
        table.insert(base, {name = "Cargo Hold (relative)"%_t, key = "cargo_hold_relative", value = round(baseCargoPerc * 100), comp = UpgradeComparison.MoreIsBetter})
    end
    if cargoPerc > 0 then
        table.insert(bonus, {name = "Cargo Hold (relative)"%_t, key = "cargo_hold_relative", value = round((cargoPerc - baseCargoPerc) * 0.5 * 100), comp = UpgradeComparison.MoreIsBetter})
    end
    if baseCargoFlat > 0 then
        table.insert(base, {name = "Cargo Hold"%_t, key = "cargo_hold", value = round(baseCargoFlat), comp = UpgradeComparison.MoreIsBetter})
    end
    if cargoFlat > 0 then
        table.insert(bonus, {name = "Cargo Hold"%_t, key = "cargo_hold", value = round((cargoFlat - baseCargoFlat) * 0.5), comp = UpgradeComparison.MoreIsBetter})
    end
   
    local rdReach, dsReach, hsReach = getHyperBonuses(seed, rarity, true)
    if hsReach > 0 then
        table.insert(base, {name = "Jump Range"%_t, key = "jump_range", value = round(hsReach * 100), comp = UpgradeComparison.MoreIsBetter})
    end
    if rdReach > 0 then
        table.insert(base, {name = "Radar Range"%_t, key = "radar_range", value = round(rdReach * 100), comp = UpgradeComparison.MoreIsBetter})
    end
